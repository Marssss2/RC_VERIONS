...
// -------------------------------------------------------
// CALIBRATION STATE (for UI polling)
// 0=idle, 1=waiting white, 2=sampling white,
// 3=waiting black, 4=sampling black, 5=done
// -------------------------------------------------------
...
void autoCalibrate() {
  calState = 1;
  calStatusMsg = "Place on WHITE surface...";
  calProgress = 0;
...
// -------------------------------------------------------
// GAMEPAD HTML — RC_CODEv10 (MERGED)
// v9: Sensitivity slider + Calibrate progress bar
// doc2: Live sensor display + Lap table + Commands ref
// -------------------------------------------------------
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;-webkit-user-select:none;user-select:none;}
    ...
    .sensor-label{font-size:9px;color:#666;margin-top:2px;text-align:center;}
  ...
  <div class="panel">
    ...
    <div class="section">
      ...
      <div class="sensor-row">
        <div><div class="sensor-box" id="sL">L</div><div class="sensor-label">LEFT</div></div>
        <div><div class="sensor-box" id="sC">C</div><div class="sensor-label">CENTER</div></div>
        <div><div class="sensor-box" id="sR">R</div><div class="sensor-label">RIGHT</div></div>
      </div>
    </div>
...
<script>
  ...
  function setSpd(v){
    document.getElementById('spdLbl').innerText=v;
    document.getElementById('spdVal').innerText=v;
    sc('set speed '+v);
  }
  function setSens(v){
    document.getElementById('sensLbl').innerText=v;
    document.getElementById('sensVal').innerText=v;
    sc('set threshold '+v);
  }
  ...
  function doSensorTest(){
    fetch('/sensordata')
      ...
      .then(d=>{
        ...
        setStatus('📡 L='+d.rawL+' C='+d.rawC+' R='+d.rawR+' | Thresh=<span>'+d.thresh+'</span>');
...
void handleCmd(AsyncWebServerRequest *request) {
  if (request->hasParam("v")) {
    ...
    if      (cmd=="f") { lineTrackingMode=false; moveForward(); }
      ...
      if (spd>=50&&spd<=255) { BASE_SPEED=spd; HARD_TURN_SPEED=constrain(spd+30,0,255); }
    }
    else if (cmd.startsWith("set threshold ")) {
      int t=cmd.substring(14).toInt();
      if (t>=100&&t<=4000) SENSOR_THRESHOLD=t;
    }
  }
...
// -------------------------------------------------------
// CALIBRATION STATUS ENDPOINT → /calstatus
// -------------------------------------------------------
void handleCalStatus(AsyncWebServerRequest *request) {
  String json="{\"state\":"+String(calState)+
              ",\"msg\":\""+calStatusMsg+"\""+
              ",\"progress\":"+String(calProgress)+
...
void recvMsg(uint8_t *data, size_t len) {
  ...
  if (message=="control") {
    WebSerial.println("[GAMEPAD] >> http://192.168.4.1/control <<");
  }
    ...
    WebSerial.println("  set ssid <name> | set pass <password>");
  }
}
